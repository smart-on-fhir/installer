...
* `{host}:9075/api/smartdstu2/data` for a FHIR DSTU2 API server
* `{host}:9075/api/smartdstu2/data/metadata` for the FHIR DSTU2 API conformance
* `{host}:9076/api/smartstu3/data` for a FHIR STU3 API server
* `{host}:9076/api/smartstu3/data/metadata` for the FHIR STU3 API conformance
...
```
sudo apt-get update; sudo apt-get -y install curl git python-pycurl python-pip python-yaml python-paramiko python-jinja2 unzip libwww-perl libdatetime-perl; sudo pip install ansible==2.3.2.0; pip install --upgrade pip;
git clone https://github.com/smart-on-fhir/installer
cd installer
```
